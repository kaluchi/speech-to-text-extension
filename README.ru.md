...
Расширение использует сервисную архитектуру на основе паттерна PageObject:
...
└── другие файлы расширения
```

### Принцип работы архитектуры

1. **PageObject** - центральный координатор, который создает и предоставляет доступ к сервисам
2. **Сервисы** - инкапсулируют определенный функционал (работа с DOM, API, событиями и т.д.)
3. **Порядок инициализации** - контролируется PageObject для корректной загрузки зависимостей
4. **Единая точка доступа** - все взаимодействия проходят через PageObject (`window.page`)
